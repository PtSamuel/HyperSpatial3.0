...
Page({
  ...
  computeRelativePosition(longitude, latitude) {
    ...
    for(let i = 0; i < this.locations.length; i++)
    {
      ...
      var absoluteDistance = Math.sqrt(target_pos[0]**2 + target_pos[1]**2 + target_pos[2]**2);
      location.position = target_pos;
      console.log(location.name, location.position)
      location.distance = absoluteDistance;
  ...
  draw2D() {
    setInterval(function() {
      for(const location of Object.entries(this.locations))
      {
